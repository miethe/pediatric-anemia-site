...
|-------|------|-------|------------------|-----------|
| 1 | Derived status & scaffold ergonomics | `status` verb (derived state machine over `reviews/` files, queue/turn output, human + `--json` forms); scaffold auto-derives `subjectContentHash` via `subject.mjs`; scaffold writes file for real identity kind (still unsigned/`signature: null`) | `status` output matches validate's derived semantics on the committed `cbc_suite_v1` dry-run set + adversarial fixtures; zero new deps | task-completion-validator |
| 2 | Sign verb & validate performance | `sign` verb using `signature.mjs`/ADR-0005 custody seams, gate-aware refusal on `synthetic:false`; scoped `validate --record`/`--module` incremental path with content-hash-keyed, fail-closed reuse | sign on synthetic path round-trips against `validate`; sign on real path refuses with G1/G2 message; validate wall-time on 5-record module measurably reduced; stale-cache test proves fail-closed | task-completion-validator |
...
| R2 | `status` reimplements derived-state logic and drifts from `validate` | High | Single shared lib function (extract from validate path into `lib/`), both verbs consume it; drift test comparing verb outputs |

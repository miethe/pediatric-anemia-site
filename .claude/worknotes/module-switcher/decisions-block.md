...
|-------|------|-------|------------------|-----------|
| P1 | Module status accessor + tripwire retirement | Shared per-module signing-status accessor keyed on `module.json.status` (closed enum, `docs/architecture.md:221`), fail-closed: any value other than a recognized signed state — and today NO state is signed; `unsigned-stub` AND unknown/absent ⇒ `unsigned-proposal` — plus `approvedBy` emptiness surfaced; retire the two explicit tripwires (`src/modules/registry.js:39-50` comment, `tests/module-registry.test.mjs:20-24`); verify all 4 modules load + assess end-to-end via existing `assess(input, moduleId, …)` (`src/engine.js:19`), incl. kidney/growth empty-rules path | Accessor unit-tested incl. fail-closed unknown-enum default; per-module assess smoke incl. empty-rules modules | `npm test` green on new suites; no clinical JSON touched |

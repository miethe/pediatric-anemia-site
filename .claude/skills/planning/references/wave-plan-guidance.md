...
| `phases[].profile` | `string` | Optional | — | Default profile for the provider in this phase. Examples: `free-tier` (ICA), `sandbox=read-only` (Codex), `web-search=on` (Gemini). Per-task `Profile` column overrides. |

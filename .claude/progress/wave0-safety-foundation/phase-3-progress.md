...
tasks:
- id: EP3-T1
  description: 'schemas/evidence.schema.json: new schema formalizing passage-level
    records — sourceLocator {page/section/table/figure}, exactPassage, evidenceGrade,
    applicability {age/sex/assay}, reviewDate, supersedes, surveillanceQuery, status
    (source-supported/implementation-proposal).'
  status: completed
  assigned_to:
  - general-purpose
  dependencies:
  - Entry (DEF-1 resolved via EP0-T6)
...
- id: EP3-T2
  description: 'rf-bundle -> KB-pack converter (OQ-2 resolved: lives in this repo,
    ...
    the verified RF-EV-001 bundle (48 claims: 35 supported/8 inferred/5 speculation).'
  status: completed
  assigned_to:
  - general-purpose
  dependencies:
...
- id: EP3-T3
  description: 'Backfill passage records for all 6 evidence sources (AC-WP3-ENUM):
    ...
    REG-002 (launched EP0-T8) gates verbatim-quote vs. paraphrase-only wording — paraphrase-only
    until REG-002 clears.'
  status: completed
  assigned_to:
  - general-purpose
  dependencies:
...
- id: EP3-T6
  description: 'R-P2 resilience — consumers handle absent evidence fields (AC-WP3-RESIL):
    ...
    A legacy-shape record encountered mid-migration must not throw.'
  status: completed
  assigned_to:
  - documentation-writer
  dependencies:

...
**Verb surfaces.** The tool exposes seven verbs:

- `list` — prints a per-module review-record state summary (records by role, informational chain linkage, synthetic flags); never a fail-closed judgment — that is `validate`'s job.
- `scaffold` — builds a draft review record (still unsigned, `signature: null` on real-identity records); with `--draft`, stages that same record to `.review-drafts/<moduleId>/` (outside `reviews/`, gitignored, uncommitted) regardless of its `synthetic` flag, for `sign` to consume.
- `sign` — reads exclusively from a `scaffold --draft`-staged file and performs this tool's one committed write into `reviews/` (TESTKEY-only synthetic path, fail-closed pre-gates G1/G2 on real records); a human still performs the actual `git add`/`git commit` afterward — neither verb runs git.
...
- `status` — reports derived review-chain state (per-module queue/turn state, next-expected role, blockers) — the turn-taking lifecycle label `status` itself computes, layered on top of the same shared module-wide `blockers` `validate` also consumes, so the two verbs' fail-closed findings can never diverge.
- `dry-run` — composes scaffold's draft-staging, sign's signing, and validate's chain-validation into a single synthetic end-to-end pass over all five ADR-0004 roles.
...
**Derived-state model.** A single `lib/derived-state.mjs#computeDerivedReviewState` function produces one structured, module-wide, fail-closed result — `{ blockers: string[], chainReport }` — consumed by both `validate` and `status`, eliminating the risk of their module-wide findings diverging. It does not itself compute a lifecycle label: `status` (`lib/verbs/status.mjs`) owns the turn-taking state machine built on top of those shared `blockers` — deriving `not-started`/`in-progress`/`disputed` from which roles have an effective act on record, and resolving `structurally-non-qualifying` (always the terminal state for synthetic records) or `acts-complete-unauthorized` (the terminal all-real state, never emitted as "release-ready") from `blockers`, collapsing to `invalid` whenever any other blocker is present. Machine-readable `blockers[]` map 1:1 onto `validate`'s own violations, so the two verbs' diagnostics stay consistent even though only `status` renders them into a state label.

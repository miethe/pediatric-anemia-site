...
| P1-T7 | Rights-leakage grep gate across all 4 fixtures (R-4 mitigation) | Per FR-3 / decisions block Risk 4: add a CI-runnable check (`npm run check` step or a dedicated script invoked by it) that verifies the ADR-0002 redaction disposition across `tests/fixtures/rf-{ev-001,cbc-002,kid-001,gro-002}/` by two mechanisms: (1) STRUCTURAL — inside each fixture's `sources/`, every `quote:`/`passage_locator:` construct must be exactly the redaction placeholder; (2) QUOTED-SPAN HASH SCAN — outside `sources/`, every double-quoted span is hashed against the placeholder-derived restricted-hash registry. **Scope limits, stated here deliberately (see `.claude/findings/multi-bundle-conversion-e1-findings.md`): bare unquoted prose is NOT inspected, bare `locator` values are exempt by design (a short selector legitimately repeats its own passage text), and `modules/**` is not scanned at all.** This is not byte-level absence proof (cross-referenced against each fixture's `HASH-PROVENANCE.md` restricted-passage list). This closes decisions block Risk 4 structurally, not only by convention. | The grep gate runs as part of `npm run validate` (or an equivalent check task) and runs in `npm run validate` (therefore `npm run check`) and fails non-zero when a restricted passage's text reappears inside a double-quoted span outside `sources/`, or when any in-`sources/` `quote:`/`passage_locator:` construct is not the exact placeholder. Verified empirically: a real restricted passage seeded as a double-quoted span exits 1 (caught); the same text appended as bare unquoted prose exits 0 (NOT caught, by design — see scope limits above) | 0.75 | node-tooling engineer | sonnet | adaptive | P1-T3, P1-T4, P1-T5, P1-T6 |
| P1-GATE | `task-completion-validator` gate | Verify Phase 1 exit gate (scoped per Decisions Block Addendum A1): `inspect` exits 0 for `rf-cbc-002`; the other 3 fixtures' `DecisionsNotFoundError` exit is expected (DF-E1-M1), not a blocker; fixture generator is unit-tested; grep gate active and passing; the rights-leakage gate passes within its documented scope (structural placeholder verification inside `sources/`; quoted-span hash scan outside it) — NOT a byte-level absence proof. | All exit-gate criteria pass (as scoped); recorded in phase progress note | — | task-completion-validator | sonnet | adaptive | P1-T1..T7 |
...
- [ ] Rights-leakage gate is active in `npm run check`/`npm run validate` and passes **within its documented scope** (structural placeholder check inside `sources/`; quoted-span hash-registry scan outside it; bare prose, bare `locator` values, and `modules/**` deliberately out of scope)

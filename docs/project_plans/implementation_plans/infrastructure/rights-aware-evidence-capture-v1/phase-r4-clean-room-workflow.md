...
`package.json` is untouched (EP-R0 barrier); the brief generator is invoked through the **`rights:brief`**
npm script EP-R0 already landed in EPR0-T6. This phase supplies the script's target file
(`scripts/rights/build-decision-brief.mjs`); it does not add or rename an npm entry.

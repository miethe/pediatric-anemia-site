...
- **No-go:** any rule without a passage or proposal flag; any dangerous-miss test regression; unsigned/expired KB not rejected by the server.
  - **Annotation (2026-07-21, SPIKE-006 RQ6/amended-design)**: this criterion predates SPIKE-006's NO-GO on cryptographic signing. The illustrative manifest above carries a `signature` field this program is not building for Phase 1 (P1-WP5) — reading "unsigned" as satisfied by `clinicalContentHash` + the new `governanceHash` (SPIKE-006's amended design) failing closed, rather than by a cryptographic signature, is a redefinition of this criterion, not an implementation detail, and is recorded here as such rather than by silent substitution. "Expired" is likewise not yet satisfied: SPIKE-006 records `docs/architecture.md` §10's expiry clause as **not closed** — see SPIKE-006's Amendment 4 for the minimum governance policy EP5-T6 must implement against, and the requirement that the staleness window be read from a declared config value rather than hardcoded.

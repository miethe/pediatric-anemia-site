...
|---|---|---|---|---|
| R1 | `RadioNodeList` guards throughout `src/app.js` (`checked()`/`checkedValues()`/`setSimpleField()`) bail out silently on encountering a `RadioNodeList`. A radio-group implementation would silently break both read and write for every affected field. | High | Would-be, if radios were chosen | Avoided structurally by choosing `<select>` (FR-1/FR-3) — a select is always a single element, never a `RadioNodeList`. Verify this explicitly during implementation, not just by design intent. |
| R2 | The safety-reviewed mutual-exclusion logic (`src/app.js:1634-1650`) force-writes a boolean `.checked = false` across every `immediateSafetyNames` field — this is safety-adjacent code and must not be rewritten as an incidental side effect of the control swap. | High | Certain — the rewrite is required (FR-10) | Treated as its own task with its own review in the implementation plan, not folded silently into the general read/write rewrite. |
| R3 | The neutrality precondition (§4) erodes silently the moment a future rule author writes an `is-absent`/`is-unknown`/`is-not-assessed` condition against one of the 14 aggregate facts. | High | Low near-term, but the failure mode is silent | The FR-9 guard test (§6, §8) — non-negotiable, written before any behavior change so it can catch this at authoring time, not in production. |
| R4 | The existing safety test `tests/tristate-safety-invariant.test.mjs:270` ("unknown is neither present nor absent, and all absence spellings normalize to unknown") hardcodes `TRI_VALUES = new Set(['true', 'false', 'unknown'])` (`:35`) — exactly three values. | Medium | Low | This feature adds no 4th wire value, so this test should stay green unmodified. **If it goes red, that is a signal the design drifted from §3 — do not edit the test to match the code; stop and re-examine the implementation against this PRD.** |
| R5 | Verification-ceiling overclaim: presenting a source-shape pin or a hand-rolled DOM shim as proof of rendering/click/keyboard/a11y correctness. | Medium | Medium without an explicit gate | §8 enumerates the ceiling explicitly and forbids the DOM-shim-as-verification pattern outright. |
| R6 | Survey fatigue: 59 dropdowns defaulting to "not assessed" may increase clinician abandonment versus 59 checkboxes defaulting to unchecked/false. | Medium | Unknown — no data | Out of this PRD's power to measure. Record as a human-factors validation item for a later validation gate; this PRD does not claim a UX improvement, only capture-fidelity. |
| R7 | `npm run check` is currently **RED on `main`** with 8 pre-existing failures (byte-identity baselines + D1 rights checks), unrelated to this work. | High (process) | Certain — already true | This plan's gate criterion is **"no NEW failures versus the recorded main baseline of 8"** (§11). This PRD does not take ownership of fixing the pre-existing 8, and does not treat their continued presence as this feature's failure. |

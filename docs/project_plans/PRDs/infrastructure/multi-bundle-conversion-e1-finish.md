...
| FR-F11 | For `anemia`, `kidney_suite_v1`, `growth_suite_v1` (all three lacking any `approved_for_rule_draft` decision by design, per FR-F5), `propose` MUST emit: `evidence.json` source records, `evidence-assertions.json` exact-passage projections, conflict-visible objects, `unresolved.json` entries, `pack-provenance.json`, `conversion-report.json` (carrying the named refusal reason), `semantic-diff.json`, an **inert, empty** `rule-proposals.json` (`proposals: []`, its wrapper's `moduleId` field set to the TARGET module's own id — never `cbc_suite_v1`'s, never any other module's), and an **inert, empty** `candidates.json` (the bare `{}` object — the SAME already-committed, repo-wide convention `modules/kidney_suite_v1/candidates.json` and `modules/growth_suite_v1/candidates.json` already use today; `candidates.json` carries no top-level `moduleId` field for ANY module in this repo, so its module-scoping is proven negatively — zero candidate ids or content belonging to any other module — not via a `moduleId` field check on that file). **Binding resolution (locked; resolves a planning-gate BLOCKING finding that this requirement's prior wording contradicted P3-T1's own AC — apply consistently everywhere this requirement is restated, including the implementation plan's P3-T1 row):** for a module with zero `approved_for_rule_draft` decisions, BOTH `rule-proposals.json` AND `candidates.json` are ALWAYS present, never omitted and never populated from another module's content — an empty-but-present artifact is testable (`proposals.length === 0 && moduleId === <target>` for the former; `Object.keys(doc).length === 0` for the latter), whereas an absent file is indistinguishable from a converter that silently skipped the step. `rules.json` and `rule-provenance.json` remain strictly ABSENT (never emitted) for these three modules — that part is unchanged from the prior wording. | Must | Decisions block §0 locked scope decision — binding; planning-gate BLOCKING finding (FIX 3) |

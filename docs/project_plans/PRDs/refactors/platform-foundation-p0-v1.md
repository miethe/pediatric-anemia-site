...
| :-: | ----------- | :------: | ----- |
| FR-1 | Define the `modules/<id>/{rules.json, candidates.json, evidence.json, reference-ranges.json, facts.<id>.js, module.json}` package contract; `git mv` the four anemia KB JSON files from `data/` into `modules/anemia/` with empty content diff. Because the move breaks every current hard-coded consumer immediately, this requirement also covers the mechanical literal-path swap (`data/x.json` → `modules/anemia/x.json`, plan task P1-T3) across the 6 hard-coded read sites — `server.mjs`, `scripts/validate-kb.mjs`, `scripts/build-static.mjs`, `tests/engine.test.mjs`, `src/app.js` (2 `fetch()` calls), `scripts/smoke-test.mjs` — as a **path-literal-only** change with no new iteration/registry logic (that logic is FR-5's job; FR-5 later replaces these literals with `MODULE_IDS`-driven iteration). This is what keeps `npm run check` green at the P1 boundary, satisfying AC-2. Capture `tests/golden/*.json` fixtures from `assessPediatricAnemia()` output for all 6 `examples/*.json` **before any code moves**, and add the permanent `tests/module-equivalence.test.mjs`. | Must | Maps to P0-WP1. Per SPIKE-001 RQ5/RQ1, plus the sequencing clarification in the implementation plan's Sequencing Note 1. `data/algorithm-explainers.json` stays at `data/` (OQ-4, binding — not KB content, not in WP1 scope). |
...
| FR-5 | Extend `src/modules/registry.js` — the file's export surface is the **union** of SPIKE-001's engine-facing API (`getModule(id)`, `listModules()`, authored in FR-3/P3 as the immediate dependency of `assess()`) and SPIKE-002's build-facing API added here: `MODULE_IDS`, `DEFAULT_MODULE_ID`, `MODULE_CODE_LOADERS` (literal enumerated `import()` specifiers), `loadModuleCode(id)`, `isRegisteredModule(id)`. FR-5 extends the file **additively** — it does not overwrite or restructure P3's `getModule`/`listModules` exports. `scripts/validate-kb.mjs`, `scripts/build-static.mjs`, `scripts/smoke-test.mjs`, and `server.mjs` are switched from FR-1's mechanical path literals to `MODULE_IDS`-driven iteration. `src/app.js`'s fetch-literal path change (`./data/{rules,candidates}.json` → `./modules/anemia/{rules,candidates}.json`) is made once, at FR-1/P1 (it must land there so `npm run check` stays green at the P1 boundary — see FR-1); FR-5 is the **only** requirement permitted to change the *logic* around those paths (hard-coded literal → registry-driven iteration), not the literal string value itself, which FR-1 already set. No further edit to `src/app.js`/`src/algorithmExplorer.js` beyond FR-1's path swap is permitted in this refactor. **No public `moduleId` field is added to `GET /api/v1/knowledge-base` or `POST /api/v1/assess` in P0** (binding decision, §7 OQ-2) — module iteration is internal only; existing request/response shapes and `openapi.yaml` are untouched. | Must | Maps to P0-WP5. Per SPIKE-002 (all RQs) and the implementation plan's Sequencing Notes 1–2. Directory-scan and dynamic-`import()`-by-variable designs are explicitly rejected (SPIKE-002 §Alternatives) in favor of the static, literal registry. |

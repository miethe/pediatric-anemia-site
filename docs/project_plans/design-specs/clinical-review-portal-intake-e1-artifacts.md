...
| Deferred/eligible-but-unrouted claim entry | `modules/<id>/unresolved.json`, entries with `entryKind: "claim"` (kidney) or `kind: "deferred_claim"` (growth) | `kidney_suite_v1` (82 claim entries as of this writing), `growth_suite_v1` (79 `supported`-status + 10 `inference`-status entries = 89) | **Hand-derived**, same generators as above | 171 total across the two modules |
...
| In-evidence `conflictsWith[]` annotations | `modules/<id>/evidence.json`, per-source `conflictsWith[]` arrays (e.g. `cbc_suite_v1`'s `CALIPER2020_HEMATOLOGY_I`, `BJHAEM2010_SCNIR_LEUKEMIA_RISK`, `PEDS2020_ISOLATED_NEUTROPENIA_OUTCOMES`; reflected as `conflictClasses` in the multi-bundle report) | `cbc_suite_v1` (3 sources / 3 conflict references), `kidney_suite_v1` (3 sources / 5 conflict references), `growth_suite_v1` (3 sources / 5 conflict references) | **Split, not uniform** — `cbc_suite_v1`'s `evidence.json` is **converter-adjacent**: `RF-CBC-002` is the one bundle whose conversion pipeline ran end-to-end via the committed converter. `kidney_suite_v1`'s and `growth_suite_v1`'s `evidence.json` `conflictsWith[]` arrays are **hand-derived** by the same bespoke, uncommitted generator scripts as their `unresolved.json` files above, not by the converter. Across all three modules this is a *different, simpler, inline* conflict-annotation shape (a string array on an evidence source record) than the `unresolved.json` named-conflict-object shape above; it is not the same artifact type and should not be conflated with it in any future portal schema. | 13 total (9 sources across 3 modules carry a non-empty `conflictsWith[]`; some sources list more than one conflicting reference) |
...
4. **Route `unresolved.json` claim entries as a triage queue, not a review queue.** Most entries
   ...
   present 171 individual claim rows to a clinical reviewer as if each needs a clinical decision today.
